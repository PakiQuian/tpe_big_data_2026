...
# Silver-like schema feeding build_gold_cost_anomaly (flags + per-method scores).
ANOMALY_SILVER_SCHEMA = StructType(
    [
        StructField("org_id", StringType()),
        StructField("service", StringType()),
        StructField("event_date", DateType()),
        StructField("org_name", StringType()),
        StructField("cost_anomaly_flag", BooleanType()),
        StructField("flag_zscore", BooleanType()),
        StructField("flag_mad", BooleanType()),
        StructField("flag_ptiles", BooleanType()),
        StructField("flag_negative", BooleanType()),
        StructField("score_zscore", DoubleType()),
        StructField("score_mad", DoubleType()),
        StructField("score_ptiles", DoubleType()),
        StructField("score_negative", DoubleType()),
    ]
)


def test_gold_cost_anomaly_scores_per_method(spark):
    d = dt.date(2025, 8, 1)
    rows = [
        # org/service/day group: two flagged events, zscore + ptiles fire.
        # (all score columns present; only the ones whose flag is True count)
        (
            "o1", "compute", d, "Org One",
            True,  True,  False, True,  False,  # flags: zscore, ptiles
            5.0,   0.0,   2.0,   None,          # scores
        ),
        (
            "o1", "compute", d, "Org One",
            True,  True,  False, False, False,  # flags: zscore only
            8.0,   0.0,   0.0,   None,
        ),
        # a NON-flagged event in a different group is excluded from the mart
        (
            "o1", "storage", d, "Org One",
            False, False, False, False, False,
            0.1,   0.1,   0.0,   None,
        ),
    ]
    df = spark.createDataFrame(rows, schema=ANOMALY_SILVER_SCHEMA)
    out = {(r["service"], r["event_date"]): r for r in cpa.build_gold_cost_anomaly(df).collect()}
